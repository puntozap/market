...
class PurchasesController extends \TCG\Voyager\Http\Controllers\Controller
{
    ...
    public function searchAndCreateInvoice(Request $request){
        $Purchase=Purchase::where("user_id",$request->user_id)->where("if_proccess_to_invoice",null)->get();
        $User=User::find($request->user_id);
        if(count($Purchase)==0)
            return response()->json([
                "success"=>false,

                "message"=>"the client '".$User->name."' has not made a purchase. try other client"
            ]);
        $Invoice=new Invoice;
        ...
        foreach($Purchase as $purchase){
            $InvoicePurchaseClient= new InvoicePurchaseClient;
            $InvoicePurchaseClient->invoice_id=$Invoice->id;
            $InvoicePurchaseClient->user_id=$Invoice->user_id;
            $InvoicePurchaseClient->product_id=$purchase->product_id;
            $InvoicePurchaseClient->quantity=$purchase->quantity;
            $InvoicePurchaseClient->save();

            $_purchase=Purchase::find($purchase->id);
            $_purchase->if_proccess_to_invoice=1;
            $_purchase->save();
        }
        ...
        return response()->json([
            ...
            "message"=>"the client '".$User->name."' has been proccessed all purchases.",
